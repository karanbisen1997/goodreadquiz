...
def download_screenshots_of_reddit_posts():
    ...
    Path("assets/temp/png3").mkdir(parents=True, exist_ok=True)
    ...
    with sync_playwright() as p:
        ...
        if len(url)>10:
            ...
            f= open("youtube_comics/my-girlfriend-is-a-zombie-comics-wuxia-dao-novel.txt", 'r')
            url = f.readlines()[-1]
            f.close()
        ...
        if  len(multi)!=0:
            ...
            for i in range(0, len(multi)-1):
                page.locator(f"(//div[@class='page-break no-gaps'])[{count+2}]").screenshot(path=f"assets/temp/png3/comment_{count}.png")
...
while 1:
    ...
    if comment:
        ...
        for i in range(0, number_of_clips):
            EFFECT_DURATION = 8
            CLIP_DURATION = 20
            clips.append(ImageClip(f"assets/temp/png3/comment_{i}.png").set_duration(CLIP_DURATION).resize(height=H).set_opacity(new_opacity))
    ...
    fd=open("youtube_comics/my-girlfriend-is-a-zombie-comics-wuxia-dao-novel.txt","r")
    d=fd.read()
    fd.close()
    m=d.split("\n")
    s="\n".join(m[:-1])
    fd=open("youtube_comics/my-girlfriend-is-a-zombie-comics-wuxia-dao-novel.txt","w+")
    ...
    # https://wuxiaworld.site/novel/my-girlfriend-is-a-zombie-comics-wuxia-dao-novel/chapter-1/
    time.sleep(10.5)